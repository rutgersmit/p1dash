...
export function HourGraph({ history }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const PAD_TOP    = 16 * dpr;
      const PAD_BOTTOM = 22 * dpr;
      const PAD_LEFT   = 48 * dpr;
      ...
      for (let i = -2; i <= 2; i++) {
        ...
        ctx.setLineDash(v === 0 ? [] : [3 * dpr, 3 * dpr]);
        ctx.beginPath();
        ...
        ctx.fillText(label, PAD_LEFT - 5 * dpr, y);
      }
      ...
      for (let m = 0; m <= 60; m += 15) {
        ...
        ctx.fillText(label, x, PAD_TOP + plotH + 5 * dpr);
      }
      ...
      if (visible.length < 1) { rafRef.current = requestAnimationFrame(draw); return; }
      ...
      // Line (two-color, clipped at zero)
  ...
  return (
    <div className="card graph-card">
      <div className="graph-header">
        <span className="graph-title">Power — last 60 min</span>
      ...
      <div className="graph-canvas-wrap graph-canvas-wrap--hour">
